  ...
  <body>
    <div class="container mt-4">
        <div class="form-container mx-auto" style="max-width: 800px;">
            <h1 class="mb-4"><i class="fas fa-user-edit me-2"></i>Edit User Record</h1>

            <%
              ArrayList<String[]> records = (ArrayList<String[]>) request.getAttribute("records");
              if (records != null && records.size() > 0) {
                  String[] user = records.get(0);
            %>

            <form action="UsersUpdateServlet" method="post">
                <div class="mb-3">
                    <label for="recordUserID" class="form-label">User ID</label>
                    <input type="text" class="form-control" id="recordUserID" name="user_id" disabled value="<%= user[0] %>">
                </div>
                
                <div class="mb-3">
                    <label for="fullname" class="form-label">Full Name</label>
                    <input type="text" class="form-control" name="fullname" id="fullname" value="<%= user[1] %>">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="recordRole" name="role" required>
                        ...
                        <option value="Admin" <%= "Admin".equals(user[2]) ? "selected" : "" %>>Admin</option>
                        <option value="Employee" <%= "Employee".equals(user[2]) ? "selected" : "" %>>Employee</option>
                        <option value="Dentist" disabled title="Dentist records can be edited in the Dentist Tab">Dentist</option>
                    </select>
                </div>

                <div class="mb-3">
                    <label for="username" class="form-label">Username</label>
                    <input type="text" class="form-control" name="username" id="username" value="<%= user[3] %>">
                </div>
                
                <div class="mb-3">
                    <label for="password" class="form-label">Password</label>
                    <input type="text" class="form-control" name="password" id="password" value="<%= SecurityServlet.decrypt(user[4]) %>">
                </div>

                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" class="form-control" name="email" id="email" value="<%= user[5] %>">
                </div>
                
                <div class="mb-3">
                    <input type="hidden" class="form-control" name="old_email" id="old_email" value="<%= user[5] %>">
                </div>

                <div class="d-flex justify-content-end mt-4">
                    <a href="UsersDisplayServlet" class="btn btn-secondary me-2">Cancel</a>
